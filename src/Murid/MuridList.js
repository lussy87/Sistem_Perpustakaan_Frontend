...
export default function StafsList() {
  ...
  const [dbo_murid, setMurid] = useState([]);
    ...
    const getUser = async () => {
        const dbo_murid = await axios.get('http://192.168.0.162:8080/murid');
  ...
  const handleCloseUpdate = () => {
    ...
    const MuridDelete = async (id) => {
      await axios.delete(`http://192.168.0.162:8080/murid/${id}`);
      getUser();
    };
          ...
          <Table sx={{ minWidth: 650 }} aria-label="simple table">
            ...
            {dbo_murid.map((murid, index) => (
                ...
                >
                  ...
                  <TableCell align="left">{murid.jk_murid}</TableCell>
                  <TableCell align="left">{murid.alamat_murid}</TableCell>
                  <TableCell align="left">{murid.no_telp_murid}</TableCell>
                ...
                <Button startIcon={<EditIcon />} color="success" onClick={() => UpdateMurid(murid.murid_id)}></Button>
